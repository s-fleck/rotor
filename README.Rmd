...
rotation. `backup()`, `backup_date()` and `backup_time()` do the same but keep the 
original file. 
...
Before we run the examples for rotor, we have to setup a temporary directory 
...
list_backups(tf)  
```
`rotate()` also makes a copy, but replaces the original file with an empty one.
...
`max_backups` parameter. By setting it to `0`, we delete all backups.
```{r}
...
* [dint](https://github.com/s-fleck/dint): A toolkit for working year-quarter 
  and year-month dates that I am also the author of. Is used by `rotate_date()` 
  and `rotate_time()` to deal with calendar periods. 
