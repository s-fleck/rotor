...
**rotor** relies solely on information encoded in the filename suffix of the
backups (i.e. the timestamp or index). It therefore also works with backups 
...
See the [function reference](https://s-fleck.github.io/rotor/reference/index.html) for 
more details
...
You can also set a maximum number of backups to be kept. Notice how the zipped
backup we created above moves to index 4 as we create three new backups.
```{r}
...
that no new backups is created, it works equivalent to using `backup()` with the
`max_backups` paramter. If we set `max_backups` to `0`, we can clean up all 
backups.
```{r}
