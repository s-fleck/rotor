...
test_that("backup_interval works as expected for years", {
  tf <- file.path(td, "test.log")
  saveRDS(iris, tf)

  # no backup younger than 1 year exists, so rotate
  bu <- backup_interval(tf, "1 year")
  ...
  # ensure backup_interval believes it is 2019-01-01
  mockery::stub(backup_interval, "Sys.Date", as.Date("2019-01-01"))
  ...
  bu <- backup_interval(tf, "2 year")  # dont rotate
  expect_true(length(bq$backups) == 1)
  bu <- backup_interval(tf, "1 year")  # rotate
...
test_that("backup_interval works as expected for quarters", {
  tf <- file.path(td, "test.log")
  saveRDS(iris, tf)
  bq <- BackupQueueDate$new(tf)

  # no backup younger than 1 quarter exists, so rotate
  bu <- backup_interval(tf, "1 quarter")
  expect_true(file.size(bu) > 1)
  expect_true(length(bq$backups) == 1)
  bq$prune(0)

  # ensure backup_interval believes it is 2019-01-01
  mockery::stub(backup_interval, "Sys.Date",    as.Date("2019-04-01"))
  mockery::stub(backup_interval, "dint::Sys.date_yq", dint::date_yq(2019, 2))
  ...
  bu <- backup_interval(tf, "2 quarter")
  expect_true(length(bq$backups) == 1)
...
test_that("backup_interval works as expected for months", {
  tf <- file.path(td, "test.log")
  saveRDS(iris, tf)
  bq <- BackupQueueDate$new(tf)

  # no backup younger than 1 month exists, so rotate
  bu <- backup_interval(tf, "1 month")
  expect_true(file.size(bu) > 1)
  expect_true(length(bq$backups) == 1)
  bq$prune(0)

  # ensure backup_interval believes it is 2019-01-01
  mockery::stub(backup_interval, "Sys.Date",    as.Date("2019-05-02"))
  mockery::stub(backup_interval, "dint::Sys.date_ym", dint::date_ym(2019, 5))
...
test_that("backup_interval works as expected for weeks", {
  tf <- file.path(td, "test.log")
  saveRDS(iris, tf)
  bq <- BackupQueueDate$new(tf)

  # no backup younger than 1 week exists, so rotate
  bu <- backup_interval(tf, "1 week")
  expect_true(file.size(bu) > 1)
  expect_true(length(bq$backups) == 1)
  bq$prune(0)

  # ensure backup_interval believes it is 2019-01-01
  mockery::stub(backup_interval, "Sys.Date",    as.Date("2019-01-30"))  # first of week is 2019-01-28
  mockery::stub(backup_interval, "dint::Sys.date_yw", dint::date_yw(2019, 5))
